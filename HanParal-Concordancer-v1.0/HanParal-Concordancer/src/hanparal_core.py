...
def detect_target_columns(df):
    """
    Automatically detect target columns in the input corpus.
    ...
    for col in df.columns:
        ...
        if re.fullmatch(r"target_\d+", col_text):
            target_columns.append(col_text)
    ...
    if not target_columns:
        raise ValueError(
            "No target columns were found. Please include at least target_1."
        )

    return target_columns


def export_annotation_workbook_to_bytes(
    search_results,
    annotation_table,
    term_summary,
    category_options,
    ...
    """
    Export the HanParal annotation workbook as bytes.

    This is used by the Streamlit app so the user can download
    the Excel file directly from the browser.
    """
    import os
    import tempfile
    ...
    try:
        ...
        format_hanparal_workbook(temp_path)
